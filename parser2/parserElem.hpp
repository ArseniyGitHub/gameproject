...
namespace PARSER_V2 {
	...
	template <typename type> struct _Vector : public std::vector<type> {
		constexpr _Vector(size_t sz, const type* begin, size_t read = -1) {
			if (read == (size_t)-1) read = sz;
			this->resize(sz);
			type* d = this->data();
			for (size_t i = 0; i < read; i++) {
				(*this)[i] = begin[i];
	...
	struct _PackableObject_ErrSizeArg : std::exception {
		const char* what() { return "parse error - so small data packet or data is not correct"; }
	};
